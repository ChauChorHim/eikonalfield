...
def render_rays(ray_batch,
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': depth_map, 'acc_map': z_vals,
           'weights': rays_o + rays_d * depth_map[:, None]}
    if retraw:
        ret['raw'] = pts
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def config_parser():
    ...
    parser.add_argument("--testskip", type=int, default=None,
...
def find_box():
    ...
    if args.dataset_type == 'llff':
        ...
        num_ = len(i_test) // 10  # len(poses)//10
        cam_ind = i_test[::num_]  # np.arange(0,len(poses),num_)
