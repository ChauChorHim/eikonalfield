...
def get_rnd(all_rays, all_idx_inside, num_ray):
    ...
    left = num_ray - chunk_per_im * len(all_idx_inside)
    for i in range(len(all_idx_inside)):
        if left > 0:
            idx_inside = all_idx_inside[i][np.random.choice(len(all_idx_inside[i]), size=[chunk_per_im + 1], replace=False)]
            left -= 1
        else:
...
def create_models(args):
    ...
    ckpts = [os.path.join(basedir, expname, args.model_path, f) for f in
             sorted(os.listdir(os.path.join(basedir, expname, args.model_path))) if 'tar' in f]
    ...
    def forward(self, t, y):
        ...
        if len(insides) > 0:
            ...
            with torch.enable_grad():
                dn = torch.autograd.functional.vjp(lambda x: self.query_fn(x, self.model), pts_inside,
                                                   v=torch.ones_like(pts_inside[:, 0:1]), create_graph=True)
...
def render_rays(ray_batch,
    ...
    output = odeint(
        IoR_emissionAbsorptionODE(voxel_grid, network_query_fn_ior, network_ior, N_samples, step_size, bb_vals,
                                  scene_bound), y0, t_vals, method='euler')
    torch.cuda.empty_cache()
...
def train():
    ...
    f = os.path.join(basedir, expname, 'args_run_ior.txt')
    with open(f, 'w') as file:
        for arg in sorted(vars(args)):
            attr = getattr(args, arg)
            file.write('{} = {}\n'.format(arg, attr))
    if args.config is not None:
        f = os.path.join(basedir, expname, 'config_run_ior.txt')
        with open(f, 'w') as file:
    ...
    rays_rgb = np.reshape(rays_rgb, [len(images), -1, 3, 3])  # [N, H*W, 3, 3]
    ...
    inside_idx = get_mask_idx(path_mask, len(poses),
                              i_train)  # concatenating the indices of masked region in each view
    ...
    for i in trange(start, N_iters):
        ...
        # taking random rays over bounding box in all views
