...
def create_models(args):
    ...
    if len(ckpts) > 0:
        ...
        print('Reloading the weights for the NeRF and IoR MLPs')

        if "network_inside_state_dict" in ckpt and not args.no_reload:
            model_inside.load_state_dict(ckpt['network_inside_state_dict'])
            print('Reloading the weights for the inside NeRF MLP')
...
class IoR_emissionAbsorptionODE(nn.Module):
    ...
    def __init__(self,query_nerf,model_nerf,nerf_inside,query_ior,model_ior,n_samples,step_size,vals,viewdir):
...
def render_rays(ray_batch,
    ...
    output = odeint(IoR_emissionAbsorptionODE(network_query_fn,network_fine,network_inside,network_query_fn_ior,network_ior,N_samples,step_size,bb_vals,viewdir), y0, t_vals,method='euler')
...
def config_parser():
    ...
    parser.add_argument("--no_reload", default=True, action='store_true', 
                        help='do not reload weights of MLP for NeRF inside')
...
def train():
    ...
    path_mask = os.path.join(basedir, expname, 'masked_regions') # concatenating the indices of masked region in each view 
    inside_idx = get_mask_idx(path_mask,len(poses),i_train)
    ...
    N_iters = 10000 + 1 # number of total iteration
    N_rand = args.N_rand
    print('Begin')
    ...
    for i in trange(start,N_iters):
        ...
        render_kwargs_train['N_samples'] = 256 + 256*i//N_iters # starting with 256 smaples and linearly increasing it to 512 samples
        ...
        if i%args.i_weights==0:
                ...
                torch.save({
                    ...
                    'network_inside_state_dict': render_kwargs_train['network_inside'].state_dict(),
        ...
        if i%args.i_img==0:
                ...
                with torch.no_grad():
                    rgb ,extras = render(H, W,K, chunk=args.chunk, c2w=pose,retraw=False,
                                                        **render_kwargs_test)
                    
                
                testimgdir = os.path.join(basedir, expname, 'training_nerf_inside')
